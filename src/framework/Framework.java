...
public abstract class Framework {
	 ...
	 * @param actual Object - what to compare
	 * @param expected Object - what we expect
	 */
	protected static void assertEquals(Object actual, Object expected) {
		//Our own assert
		if (!actual.equals(expected)) errorHeppens("Wrong assertEquals"); 		
		Assert.assertEquals(actual, expected);
    }
	
	/**
	 * @param condition Boolean if false than test fails
	 */
	protected static void assertTrue(Boolean condition) {
		if (!condition) errorHeppens("Wrong assertTrue");
	...
	/**
	 * Custom actions with error
	 * TODO: move it away from Framework
	 * @param errorMessage
	 */
	public static void customErrorHandler(String errorMessage) { 
		...
		createAndWriteTextFile(errorMessage + NEW_LINE 
				+ driver.getCurrentUrl() + NEW_LINE 
				+ driver.getTitle() + NEW_LINE, 
				prefix);
	}
     ...
     * @param text - text to search
	 * @param element - in what element we are locking in
	 * @return true if we have such text
	 */
	protected static boolean isTextInElement(String text, By element) {
		if (driver.findElement(element).getText().contains(text)) return true;
		return false;
	}
     ...
     * @param pathToElements - selector to group of elements
     * @param elementNumber - on which element we are clicking on. Start from 1.
     */
    protected static void clickOnOneElementInGroup(By pathToElements , int elementNumber) {
    	try {
    		WebElement element = driver.findElements(pathToElements).get(elementNumber - 1);
        	element.click();
    ...
    protected static void clickOnOneAnyElementInGroup(By pathToElements) {
    	...
    	if (size == 0) {
			...
			WebElement element = driver.findElements(pathToElements).get(r.nextInt(size));
	    	element.click();
		}
	}
     ...
     * @param textInTitle - string in title
     */
    protected static void isTitleContainsAssert(String textInTitle) {
    	//Waiting for page
    	if (!driver.getTitle().contains(textInTitle)) wait(2000);
    ...
    protected static boolean isTitleContains(String textInTitle) {
    	if (driver.getTitle().contains(textInTitle)) return true;
		return false;
	}
     ...
     * @param by - what element
     * @param value - what to fill
     */
     ...
     * @param byGroup - what to fill
     * @param value - with what fill, if "" than random 6 numbers
     */
     ...
     * @param pageTitle - what page to wait
	...
	protected static boolean waitForPage(String pageTitle) { 
		for (int secondNow = 0; secondNow < WaitTime; secondNow++) {
			try { 
				if (driver.getTitle().contains(pageTitle) ) return true; 
			} catch (Exception e) {	 
			}
			wait(1000);
		}
	 ...
	 * Exception if after time put no element found
	 */
...
//        });
		...
		for (int secondNow = 0; secondNow < WaitTime; secondNow++) {
			try { 
				if (isTextOnPage(text)) return true; 
			} catch (Exception e) {	 
			}
			wait(1000);
		}
    ...
    protected static boolean waitForNotVisible(By by) {
		for (int secondNow = 0; secondNow < WaitTime; secondNow++) {
            ...
            // If results have been returned, the results are displayed in a drop down.
            if (!resultsDiv.isDisplayed()) return true;
			wait(1000);
		}
    	return false;
	}
     ...
     * @param timeToWait - what time to wait in Seconds (sec)
     */
    protected static void wait(int timeToWait) {//TODO:Do not working for HtmlUnit
    ...
    protected static boolean isElementPresentWait(By by, int timeToWait) {
    	if (timeToWait == 0) timeToWait = WaitTime;
    ...
    protected static String getText(By by) {
    	...
    	try {
    		...
    		if (text.isEmpty()) text = element.getAttribute("value"); //For input fields  
		} catch (NoSuchElementException e) {
			errorHeppens(e.getMessage());
		}
		return text;
    ...
    protected static double getDouble(By by) {
    	...
    	if (workStr.charAt(0) == '$') workStr = workStr.substring(1); 
    ...
    protected static boolean isElementPresent(By by) {
		...
		if (element != null) isPlesent = true;
		return isPlesent;
	}
